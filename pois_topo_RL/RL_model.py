...
def _unwrap_base_env(env):
    """
    Return the underlying custom env (TopoPoisoningEnv) from any SB3/Gymnasium wrappers.
    Works for:
      - DummyVecEnv([... Monitor(TopoPoisoningEnv) ...])
      - Monitor(TopoPoisoningEnv)
      - TopoPoisoningEnv
    """
    # If it's a VecEnv, take the first sub-env
    if hasattr(env, "envs"):  # DummyVecEnv/SubprocVecEnv
        e = env.envs[0]
    else:
        e = env
    # Peel Monitor / other wrappers (they expose `.env`)
    while hasattr(e, "env"):
        e = e.env
    return e
...
def learn_topo(env, reward_thrshd):
    ...
    # env is a DummyVecEnv; take the first sub-env (index 0)
    
    vec = model.get_env()  # SB3 returns the training env here
    base_env = _unwrap_base_env(vec if vec is not None else env)
    best_record = base_env.best
